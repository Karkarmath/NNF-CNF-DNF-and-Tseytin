...
-- Тут начинается кусок ещё прошлого кода.
...
--- Удаляем ненужные (тривиальные) подформулы типа Var z, Bot, Top. Делаем это, так как мы им в эквивалентность ничего ставить не будем.
...
--- Заводим новые переменные и ставим соответствующие эквивалентности (правый аргумент эквивалентности выбран новой переменной, так как при раборе формулы, всё, что стоит слева в эквивалентности, будет восприниматься как цельная формула, как и должно быть. И всё это будет эквивалентно одной новой переменной).
newVar :: [Form] -> [Form]
...
--- Итоговый алгоритм, инициализирующее значение у foldr тут -- это старшая переменна, всё так, как и должно быть. Так как на атомах он ничего делать не должен, эти случаи выписал отдельно, иначе напоремся на случай пустого списка в функции getLastVar.
